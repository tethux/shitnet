...
auto test_option_success() -> void {
    ...
    auto parsed = cli::parse<RunOptions>(8, argv);
    assert(parsed);
...
auto test_option_errors() -> void {
    {
        char executable[] = "test";
        char *argv[]{executable};
        auto parsed = cli::parse<RunOptions>(1, argv);
        assert(!parsed);
...

    {
        ...
        auto parsed = cli::parse<RunOptions>(3, argv);
        const auto *error = std::get_if<cli::unknown_option>(&parsed.error());
...

    {
        ...
        auto parsed = cli::parse<RunOptions>(2, argv);
...

    {
        ...
        auto parsed = cli::parse<RunOptions>(5, argv);
...

    {
        ...
        auto parsed = cli::parse<RunOptions>(5, argv);
        const auto *error = std::get_if<cli::invalid_choice>(&parsed.error());
        assert(error != nullptr);
        assert(error->suggestion == "json");
    }
}

auto test_multicall() -> void {
    {
        char executable[] = "test";
        char *argv[]{executable};
        auto parsed = cli::parse_command<TestRoot>(1, argv);
...

    {
        ...
        auto parsed = cli::parse_command<TestRoot>(2, argv);
        const auto *error = std::get_if<cli::unknown_command>(&parsed.error());
        assert(error != nullptr);
...

    {
        ...
        auto parsed = cli::parse_command<TestRoot>(4, argv);
        assert(parsed);

        const auto result = match(
            *parsed,
            case_of<cli::parsed_command<RunCommand>>([](const auto &command) {
                return command.options.name.value == "alpha" ? 17 : 0;
            }),
            case_of<cli::parsed_command<PlainCommand>>(
                [](const auto &) { return 0; }));
...
auto test_help_and_diagnostics() -> void {
    const auto root_help = cli::command_help_text<TestRoot>();
    const auto run_help = cli::command_help_text<TestRoot>("run");
    const auto plain_help = cli::command_help_text<TestRoot>("plain");
    ...
    const auto rendered = cli::command_error_text<TestRoot>(error, 3, argv);
